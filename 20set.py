...
# 집합 출력 2
for i in range(len(nums)):        # 순차조회 불가
    print(nums[i])
...
# 집합 값 수정 : 기본적으로는 수정 불가, 단 리스트 변환시 가능
nums[0] = 999
...
# ex) 로또 645 : 1 ~ 45 사이 임의의 숫자 6개 생성2
...
# ex) 로또 645 : 1 ~ 45 사이 임의의 숫자 6개 생성3
# 단, list를 이용해서 같은 숫자가 출현하지 않도록 작성
...
# 로또번호를 뽑을 인덱스를 난수로 생성해서 리스트에서 해당위치의 값 추출
