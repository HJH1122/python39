...
# ex) 1~100사이 정수 중 짝수만 골라서 리스트에 저장 : 소비에 관련됨
evens = []
...
# ex2) 1~10 사이 정수를 제곱해서 리스트에 저장 : 생산에 관련됨
squares = []
...
# 순서 있는 개체들을 다룰때 인덱스를 포함시켜 객체를 리턴해줌
print(names)           # 값만 출력

idx = 0
for n in names:     # 값과 순서 출력1
    print(idx, n)
    idx += 1


for idx, name in enumerate(names):     # 값과 순서 출력2
    print(idx, name)


# ex29) 사용자로부터 소문자를 입력 받아 대문자로 출력하는 프로그램을 작성해보세요.
# 단, 소문자 이외의 숫자나 대문자를 입력 받으면 “잘못 입력하셨습니다!!”라는 메시지를 출력하도록 합니다. (UpperCase)
# 대문자 A : 65, 소문자 a : 97
